...
/*
stab:
    // Программа для оценки колличества коэффициентов
    // оптимального (оптимих. метода) КИХ-фильтра
    // нижних частот или полосового фильтра, Айфичер 
...
int main(int argc, char const *argv[])
{
    ...
    ftype = filterSpec(); // filter specification
    switch (ftype)
    {
    case 1:
        N = lpKoef();
    ...
    case 2:
        N = bpKoef();
    default:
        printf("%s", "Error in type definition! \n");
        break;
    }

    printf("The number of koeficients: \t%f\n", N);
    printf("Pass delta: in dB \t%f\n", dp);
    printf("!Pass delta: in dB \t%f\n", ds);
...
int filterSpec () {
   int type;
   printf("Choose the type of filter. ");
   printf("Type '1' for define LP: ");
   printf("Type '2' for define BP: ");
   scanf("%d", &type);
   printf("\n");

   printf("Input dp and ds ");
   printf("(from 0 to 1): \n");
   scanf("%f%f", &dp, &ds);
   ...
   {
   case 1:
       printf("Input normal df: \n");
       scanf("%f", &df);
       break;
   case 2:
       printf("Input normal df (less): \n");
       scanf("%f", &df);
       break;
   }
   return type;
